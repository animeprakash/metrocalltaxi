<?php		ini_set('display_errors', 0); 
			...
			{			
				...
				$allquery="select cancelid,cancelreason,customid,cusid,cusname,cusmobile,triptype,taxiname from cancelbookings inner join bookings on customid=cusid where cusid='".$cusdata."' order by cancelid desc";
				$result= mysqli_query($con, $allquery);	
			}
			else
			{
				$query="select cancelid,cancelreason,customid,cusid,cusname,cusmobile,triptype,taxiname from cancelbookings inner join bookings on customid=cusid order by cancelid desc";
...
<div class="top">
		<div class="topbox">
			<form action="cancelledrides.php" method="post">
				<div class="searchbox">
					<input type="text" name="cusdata" id="cusdata"  placeholder=" Search Customer ID">
...
?>
	<tr>
		...
		<td class="drivertd"><?php echo $datas['cancelreason'];?></td>		
		<td><a href="deletecancelrides.php?cancelid=<?php echo $datas['cancelid'];?>" ><button class="btn btn-sm text-white bg-danger text-right" style="margin-left:120%;"> DELETE</button><td>	</a>
	</tr>
<?php } ?>
	</table>
	</div>

...
<section class="taxiscroll" >
	<div >
		...
		<?php
			ini_set('display_errors', 0);
			include("database.php");
			$query="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='airport'";
			$result= mysqli_query($con, $query);
			while($datas= mysqli_fetch_assoc($result)) {
				 $image= 'uploads/'.$datas["taxiimage"];
				//Total amount calculation 
				 $rate=$datas['taxirate'];
				 $totalamount=round(10*$rate);		
				//Total discount calculation
				$discount=$datas['taxidiscount'];
				$totaldiscount=round(10*$discount);
				//Final amount
...
<div class="carselection carrow">
		...
		<div class="column distance"> 
				<p class="distanceshow">Icludes <?php echo $distance=10;?> km</p>
		...
		<div class="column"> 
			<form action="airportbooking.php" method="GET" id="formone">
				...
				<input type="hidden" name="distance" id="distance" value="<?php echo $distance=10; ?>" >
				<input type="hidden" name="taxiname" id="taxiname" value="<?php echo $datas['taxiname']; ?>" >
				<input type="hidden" name="triptype" id="triptype" value="Airport" >
...
<?php			ini_set('display_errors', 0);
			include("database.php");
			$querythree="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='airport'";
			...
			//Total amount calculation 
				...
				$finalamount=$totalamount-$totaldiscount;
?>
			<a class="anchor" style="text-decoration:none;font-weight:bold;color:black;" href="selectairporttaxi.php?taxiid=<?php echo $datasthree['taxiid']; ?>&airporttrip=<?php echo $airporttrip; ?>&sourcecity=<?php echo $sourcecity; ?>&destinationcity=<?php echo $destinationcity; ?>&airportaddress=<?php echo $airportaddress; ?>&fromdate=<?php echo $fromdate; ?>&fromtime=<?php echo $fromtime; ?>">	
...
<?php			ini_set('display_errors', 0);
			...
			{
			$taxiid=$_GET['taxiid'];
			$querytwo="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='airport' and taxiid='".$taxiid."' ";
			$resulttwo= mysqli_query($con, $querytwo);
			}
			else{
			$querytwo="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='airport' limit 0,1 ";
			$resulttwo= mysqli_query($con, $querytwo);				
			}
		  ...
		  <div class="row">
			<div class="col-7 tble">
				<table>
					...
					<tr><td class="tdcolumn" style="font-weight:bold;font-size:13px;"><?php echo $distance=10;?> kms package </td></tr>

...
<section class="taxiscroll" >
	<div >
		...
		<?php
			ini_set('display_errors', 0);
			include("database.php");
			$query="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='oneway'";
			$result= mysqli_query($con, $query);
			while($datas= mysqli_fetch_assoc($result)) {
				 $image= 'uploads/'.$datas["taxiimage"];
				//Total amount calculation 
				 $rate=$datas['taxirate'];
				 $totalamount=round($distance*$rate);		
				//Total discount calculation
				$discount=$datas['taxidiscount'];
				$totaldiscount=round($distance*$discount);
				//Final amount
...
<div class="carselection carrow">
		...
		<div class="column distance"> 
				<div class="columntwo">
					<i class="fa-solid fa-indian-rupee-sign icons"></i><br>
					<span class="spans">Includes Toll, State Tax & GST</span>	
				</div>
				<div class="columntwo">
					<i class='fas fa-medal icons'></i><br>
					<span class="spans">Top Rated Cabs & Chauffeurs</span>	
				</div>
		</div>
		<div class="column amount"> 
			...
			<div class="columntwo">
				...
				<p class="distanceshow">up to <?php echo $distance;?> km</p>
...
<?php			ini_set('display_errors', 0);
			include("database.php");
			$querythree="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='oneway'";
			...
			//Total amount calculation 
				...
				$finalamount=$totalamount-$totaldiscount;
?>
			<a class="anchor" style="text-decoration:none;font-weight:bold;color:black;" href="selectonewaycars.php?taxiid=<?php echo $datasthree['taxiid']; ?>&sourcecity=<?php echo $sourcecity; ?>&destinationcity=<?php echo $destinationcity; ?>&fromdate=<?php echo $fromdate; ?>&fromtime=<?php echo $fromtime; ?>">	
...
<?php			ini_set('display_errors', 0);
			...
			{
			$taxiid=$_GET['taxiid'];
			$querytwo="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='oneway' and taxiid='".$taxiid."' ";
			$resulttwo= mysqli_query($con, $querytwo);
			}
			else{
			$querytwo="select taxiid,taxiimage,taxiname,taxiabout,taxirate,taxidiscount,taxitriptype,taxiactype,taxiseats,taxibags,taxifacility from taxidetails where taxitriptype='oneway' limit 0,1 ";
			$resulttwo= mysqli_query($con, $querytwo);				
			}
		  ...
		  <div class="row">
			<div class="col-7 tble">
				<table>
					...
					<tr><td class="tdcolumn" style="font-weight:bold;font-size:13px;"><?php echo $distance;?> kms package </td></tr>
